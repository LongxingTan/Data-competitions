...
def get_global_feature(
    data, id_col, target_col, agg_funs=["mean"], feature_cols=[], prefix=None
):
...
def get_rolling_feature(
    data, roll_columns, periods, id_col, agg_funs=["mean"], feature_cols=[], prefix=None
):
    ...
    for col in roll_columns:
        for lag in lags:
            for agg_fun in agg_funs:
                ...
                feature_col = (
                    prefix
                    + "_lag{}_roll{}_by_{}".format(lag, period, id_col)
                    + str(agg_fun)
                )
                feature_cols.append(feature_col)
                data[feature_col] = data.groupby(id_col)[col].transform(
