...
def build_data(base_dir, debug=False, online=False):
    ...
    if online:
        ...
        x_train = demand.loc[
            (demand["ts"] < split_date) & (demand["weekday"] == 0), feature_cols
        ]
        y_train = demand.loc[
            (demand["ts"] < split_date) & (demand["weekday"] == 0), "target"
        ]
        weight_train = demand.loc[
            (demand["ts"] < split_date) & (demand["weekday"] == 0), "weight"
        ].tolist()
...
def add_target(demand):
    demand["target"] = demand.groupby(["unit"])["demand"].transform(
        lambda x: x.shift(-21).rolling(14).sum(skipna=True)
    )  # 未来第2周到第3周的需求
...
def get_feature(data, demand_col="demand"):
    ...
    # lag roll feature
    data = get_lag_rolling_feature(
        data,
        roll_columns=[demand_col],
        lags=[0, 7, 14, 21],
        period=7,
        id_col="unit",
        agg_funs=["max", "sum", "std"],
...
def run_train(base_dir, debug=False, online=False):
    if online:
        x_train, y_train, weight_train = build_data(
            base_dir, debug=debug, online=online
        )
        ...
        (
            ...
            weight_train,
            weight_valid,
        ) = build_data(base_dir, debug=debug, online=online)
