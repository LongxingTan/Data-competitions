...
def forecast_function(demand_hist, lead_time=14, model=None):
    ...
    if args.use_model == "rule_lgb":
        return [
            0.25 * forecast_function_rule(demand_hist, lead_time, model)[0]
            + 0.75 * forecase_function_lgb(demand_hist, lead_time, model)[0]
        ]
    else:
        raise ValueError
...
def forecast_function_rule(demand_hist, lead_time=14, model=None):
    ...
    demand_sum = 1.1 * (
        1.2 * np.sum(norm[-7:])
        + 0.3 * np.sum(norm[-14:-7])
        + 0.15 * np.sum(norm[-21:-14])
    )
...
def get_safety_stock(demand_hist, lead_time, decrease=False, w1=11, w2=1.76, w3=0.09):
    ...
    if True:
        if len(pos) > 4:
            ...
            ss = (
                w1 * np.std(pos_norm)
                + w2 * np.std(demand_week.iloc[-52:]["qty"])
                + w3
                * (
                    1.0 * np.max(demand_week["qty"])
                    - np.max(np.mean(demand_week["qty"]), 0)
                )
            )
...
class TestUnit:
    ...
    def update(self, arrival_today, demand_today):
        """
        ...
        if demand_today < 0:
            self.qty_inventory_today = self.qty_inventory_today + min(
                -demand_today, self.qty_using_today
            )
        else:
            self.qty_inventory_today = max(self.qty_inventory_today - demand_today, 0.0)
        self.qty_using_today = max(
            self.qty_using_today + min(demand_today, inv_today), 0.0
        )

    def replenish_function(
        self, date, qty_demand_forecast, demand_hist, decrease=False
    ):
        """
        根据当前状态判断需要多少的补货量
        补货的策略由选手决定，这里只给一个思路
...
class ReplenishUnit:  # 填弹
    ...
    def update(self, date, arrival_today, demand_today):
        """
        每日根据当天补货到达与当日净需求更新状态
        :param date:
        :param arrival_today: 当天补货到达
        ...
        for test_number, arrival_today in arrival_today[["test_number", "qty"]].values:
            self.test_unit[test_number].update(
                arrival_today=arrival_today, demand_today=demand_today
            )
        self.demand_hist = self.demand_hist.append(
            {"ts": date, "unit": self.unit, "qty": demand_today}, ignore_index=True
        )
    ...
    def replenish_function(self, date, decrease=False):
        ...
        :return:
        """
        qty_demand_forecast = forecast_function(
            self.demand_hist, model=self.ml_model
        )  # 预测
        res = pd.DataFrame(columns=["unit", "test_number", "ts", "qty"])
        # 对每个test_number做决策
        for test_number in self.test_unit:
            repl = self.test_unit[test_number].replenish_function(
                date=date,
...
class Env:
    ...
    def update(self, unit, date, arrival_today, demand_today):
        self.replenish_unit_dict[unit].update(
            date=date, arrival_today=arrival_today, demand_today=demand_today
        )

    def replenish(self, unit, date, decrease=False):
        return self.replenish_unit_dict[unit].replenish_function(
            date=date, decrease=decrease
        )
    ...
    def replenish_logic(self, data):
        ...
        for chunk in demand_data.groupby("unit"):
            ...
            for date in date_list:
                ...
                arrival_today = arrival_data[
                    (arrival_data["ts"] == date) & (arrival_data["unit"] == unit)
                ]
                # 更新信息
                self.model.update(
                    unit=unit,
                ...
                if date.dayofweek == 0:
                    print("决策日决策中")
                    replenish = self.model.replenish(
                        unit=unit, date=date, decrease=False
                    )  #
...
if __name__ == "__main__":
    ...
    for chunk in using_hist.groupby("unit"):
        ...
        del demand["diff"]
        demand = demand[1:]
        replenishUnit_dict[unit] = ReplenishUnit(
            unit=unit,
            demand_hist=demand,
            test_unit=test_unit,
            intransit=pd.Series(
                index=date_list.tolist(), data=[0.0] * (len(date_list))
            ),
            qty_inventory_today=inventory[inventory["unit"] == unit]["qty"].values[0],
            qty_using_today=using_hist[
                (using_hist["ts"] == last_dt) & (using_hist["unit"] == unit)
            ]["qty"].values[0],
            arrival_sum=0.0,
            lead_time=lead_time,
        )
