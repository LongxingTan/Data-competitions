...
def run_train(base_dir, seed=315):
    ...
    feature_cols = [i for i in list(x_train.columns) if i not in ['SignalFileName', 'T8_CASE1', 'T8_CASE2']]
...
def run_train2(base_dir, seed=315):
    ...
    x_test[['SignalFileName', 'diff_mean', 'T8_CASE1']].to_csv('../../data/result/t8c1_res.csv', index=False)
