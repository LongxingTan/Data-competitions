...
def build_data2(base_dir):
    ...
    t9_count = t9.groupby(['SignalFileName', 'cat']).agg(count=('主轴负载', 'count'), median=('主轴负载', 'median')).reset_index()
    t9_count = t9_count.loc[(t9_count['count'] > 55) & (t9_count['median'] > 20)]

    t9c3 = t9_count.groupby(['SignalFileName']).agg(medianmean=('median', 'mean'), median_max=('median', 'max')).reset_index()
...
def run_train2(base_dir, seed=315):
    ...
    print(x_train_true['count_medianmean'].mean(), x_train_false['count_medianmean'].mean())

    x_test['T9_CASE3'] = False
    x_test.sort_values(['count_medianmean'], ascending=True, inplace=True)
    x_test.iloc[-1:]['T9_CASE3'] = True  #特殊情况
    x_test[['SignalFileName', 'count_medianmean', 'T9_CASE3']].to_csv('../../data/result/t9c3_res.csv', index=False)
