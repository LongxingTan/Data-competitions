...
def run_train(base_dir, seed=315):
    ...
    feature_cols = [
        ...
        if i not in ["SignalFileName", "T2_CASE1", "T2_CASE2", "T2_CASE3", "T2_CASE4"]
    ]
...
def run_train2(base_dir, seed=315):
    ...
    x_train_true = x_train.loc[x_train["T2_CASE3"] == True]
    x_train_false = x_train.loc[x_train["T2_CASE3"] == False]
    print(x_train_true["large12_diff"].mean(), x_train_false["large12_diff"].mean())

    x_test["T2_CASE3"] = False
    x_test.sort_values(["large12_diff"], ascending=False, inplace=True)
    x_test.iloc[:3]["T2_CASE3"] = True
    x_test[["SignalFileName", "large12_diff", "T2_CASE3"]].to_csv(
        "../../data/result/t2c3_res.csv", index=False
    )
