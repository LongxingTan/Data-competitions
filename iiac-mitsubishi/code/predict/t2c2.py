...
def run_train2(base_dir, seed=315):
    ...
    print(
        x_train_true["abs_mean_mean_abs"].mean(),
        x_train_false["abs_mean_mean_abs"].mean(),
    )

    x_test["T2_CASE2"] = False
    x_test.sort_values(["abs_mean_mean_abs"], ascending=False, inplace=True)
    x_test.iloc[:4]["T2_CASE2"] = True
    x_test[["SignalFileName", "abs_mean_mean_abs", "T2_CASE2"]].to_csv(
        "../../data/result/t2c2_res.csv", index=False
    )
