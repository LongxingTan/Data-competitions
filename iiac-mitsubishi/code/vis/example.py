...
def vis_example(file, label=None, plot=False):
    ...
    if file == '20210416-part_12.pkl':  # 训练
        example = example.iloc[list(range(4000)) + list(range(12500, example.shape[0]))].reset_index()
    if file == '20210517-part_18.pkl':  # 训练
        example = example.iloc[list(range(2000)) + list(range(16000, example.shape[0]))].reset_index()
    if file == '20210601-part_18.pkl':  # 训练
        example = example.iloc[list(range(2000)) + list(range(8000, example.shape[0]))].reset_index()
    if file == '20210528-part_9.pkl':  # 训练
        example = example.iloc[list(range(4500)) + list(range(8500, example.shape[0]))].reset_index()
    if file == '20210426-part_14.pkl':  # 验证
        example = example.iloc[list(range(4100)) + list(range(18800, example.shape[0]))].reset_index()
    if file == '20210514-part_18.pkl':  # 验证
        example = example.iloc[list(range(2000)) + list(range(10100, example.shape[0]))].reset_index()
    if file == '20210425-part_20.pkl':  # 测试
        example = example.iloc[list(range(4200)) + list(range(14000, example.shape[0]))].reset_index()
...
if __name__ == '__main__':
    ...
    for file in files:
        if label is not None:
            example_label = label.loc[label['SignalFileName'] == 'Signal Package/{}'.format(file)]
        else:
            example_label = None
