...
def find_t11_stage(load, speed):
    ...
    t11_start = np.min(np.arange(len(censored_speed))[censored_speed > 0.1]) + t2_end  # 前期如果先到过0，后面的也不需要
...
def find_t8_stage(load, speed):
    ...
    t8_start = np.min(np.arange(len(censored_speed))[censored_speed == t8_speed_mode]) + t8_end - (t11_end - t11_start)
...
def find_t9_stage(load, speed):
    ...
    t9_end = np.min(np.arange(len(censored_speed))[censored_speed > -111]) + t9_start + 900
