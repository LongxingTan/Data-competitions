...
def get_main_load(data, training=False):
    ...
    for id in np.unique(data['SignalFileName']):
        ...
        if len(temp_median) > 4:
            ...
            feature.append([id, diff_mean, diff_median, diff_max, max_min, diff_mean_40, diff01_abs, diff12_abs, diff23_abs, diff34_abs, min_count, min2_count, max_count, var_count])

    feature = pd.DataFrame(np.array(feature), columns=['SignalFileName', 'diff_mean', 'diff_median', 'diff_max', 'max_xin', 'diff_mean_40', 'diff01_abs', 'diff12_abs', 'diff23_abs', 'diff34_abs', 'min_count', 'min2_count', 'max_count', 'var_count'])
    ...
    if training:
        t8_label = pd.read_csv('../../data' + '/1 训练用/Training set.csv', usecols=range(1, 29))
