...
def prepare_train(base_dir='../../data/1 训练用', t2=False, t8=False, t9=False):
    ...
    for file in files: # ['20210601-part_18.pkl']:
        ...
        example_label = labels.loc[labels['SignalFileName'] == 'Signal Package/{}'.format(file)]

        # 数据清洗(去除负载大于50， 去除停机）(第二种类型停机只有两个，手动修改)
        example = filter_data(example, plot=False)
        if file == '20210416-part_12.pkl':  # 训练
            example = example.iloc[list(range(4000)) + list(range(12500, example.shape[0]))].reset_index(drop=True)
        if file == '20210517-part_18.pkl':  # 训练
            example = example.iloc[list(range(2000)) + list(range(16000, example.shape[0]))].reset_index(drop=True)
        if file == '20210601-part_18.pkl':  # 训练
            example = example.iloc[list(range(2000)) + list(range(8000, example.shape[0]))].reset_index(drop=True)
        if file == '20210528-part_9.pkl':  # 训练
            example = example.iloc[list(range(4500)) + list(range(8500, example.shape[0]))].reset_index(drop=True)
        if file == '20210426-part_14.pkl':  # 验证
            example = example.iloc[list(range(4100)) + list(range(18800, example.shape[0]))].reset_index(drop=True)
        if file == '20210514-part_18.pkl':  # 验证
            example = example.iloc[list(range(2000)) + list(range(10100, example.shape[0]))].reset_index(drop=True)
        if file == '20210425-part_20.pkl':  # 测试
            example = example.iloc[list(range(4200)) + list(range(14000, example.shape[0]))].reset_index(drop=True)
        ...
        if t2:
            ...
            label.append(example_label[['SignalFileName', 'T2_CASE0', 'T2_CASE1', 'T2_CASE2', 'T2_CASE3', 'T2_CASE4']])
        if t8:
            print(file)
            start, end = find_t8_stage(load, speed)
            label.append(example_label[['SignalFileName', 'T8_CASE0', 'T8_CASE1', 'T8_CASE2']])
        if t9:
            start, end = find_t9_stage(load, speed)
            label.append(example_label[['SignalFileName', 'T9_CASE0', 'T9_CASE1', 'T9_CASE2', 'T9_CASE3', 'T9_CASE4']])
