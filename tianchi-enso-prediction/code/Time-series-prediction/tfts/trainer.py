...
class Trainer(object):
    ...
    def train(
        ...
        if self.use_ema:
            self.ema = tf.train.ExponentialMovingAverage(0.9).apply(
        ...
        for epoch in range(n_epochs):
            ...
            print(
                "Epoch: {}, Train Loss: {:.4f}, Valid Loss: {:.4f}".format(
                    epoch + 1, epoch_loss, valid_epoch_loss
                )
                + ",".join(
                    [
                        " Valid Metrics{}: {:.4f}".format(i, me)
                        for i, me in enumerate(valid_scores)
                    ]
                )
            )
    ...
    def train_step(self, x_train, y_train):
        ...
        gradients = [
            (tf.clip_by_value(grad, -5.0, 5.0)) for grad in gradients
        ]  # gradient clip
        opt = self.optimizer.apply_gradients(
    ...
    def valid_loop(self, valid_dataset):
        ...
        if self.eval_metric:
            ...
            for metric in self.eval_metric:
                valid_scores.append(
                    metric(y_valid_trues, y_valid_preds, transform=self.transform)
                )
