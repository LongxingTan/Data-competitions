...
# tfts is an open-souced TensorFlow time series package, support more powerful models
# welcome to star and contribute: https://github.com/LongxingTan/Time-series-prediction

from dataset import DataLoaderTF, load_cmip, load_soda, prepare_2d_feature, prepare_2d_label
import tensorflow as tf
from tensorflow.keras.layers import Embedding, Input, Layer
from tfts import Trainer, build_tfts_model
from utils import custome_rmse_fn, eval_rmse, eval_score, set_seed


def build_data(base_dir, batch_size):
    train = prepare_2d_feature(
        data_dir=base_dir + "/enso_round1_train_20210201/CMIP_train.nc"
    )
    label = prepare_2d_label(
        label_dir=base_dir + "/enso_round1_train_20210201/CMIP_label.nc"
    )

    SODA_train = prepare_2d_feature(
        data_dir=base_dir + "/enso_round1_train_20210201/SODA_train.nc"
    )
    SODA_label = prepare_2d_label(
        label_dir=base_dir + "/enso_round1_train_20210201/SODA_label.nc"
    )

    train_data_reader = load_cmip(
        data=train, label=label, random_select=True, filter_1_99=True
    )
    valid_data_reader = load_soda(data=SODA_train, label=SODA_label, random_select=True)

    train_dataloader = DataLoaderTF(train_data_reader)(
...
class ENSO(object):
    ...
    def __init__(self):
        ...
        )  # input_dim: vocab_size, output_dim: embed_size, input_length: input sequence length
    ...
    def forward(self, sst, t300, ua, va, month):
        ...
        sst_roi_var = tf.expand_dims(
            tf.math.reduce_variance(sst_nino34, axis=(2, 3)), -1
        )

        sst_lag2 = tf.roll(sst_roi_mean, shift=2, axis=1)
        sst_mean_lag2 = tf.concat(
            [tf.zeros_like(sst_lag2)[:, :2], sst_lag2[:, 2:]], axis=1
        )
        ...
        t300_mean_lag2 = tf.concat(
            [tf.zeros_like(t300_lag2)[:, :2], t300_lag2[:, 2:]], axis=1
        )
        ...
        decoder_month_embed = tf.concat(
            [encoder_month_embed, encoder_month_embed], axis=1
        )
        decoder_quarter_embed = tf.concat(
            [encoder_quarter_embed, encoder_quarter_embed], axis=1
        )

        encoder_month = tf.one_hot(
            tf.cast(month, tf.int32), depth=12
        )  # => batch * 12 * 12
        decoder_month = tf.concat([encoder_month, encoder_month], axis=1)  # batch * 24
        decode_idx = tf.tile(
            tf.expand_dims(tf.range(24), 0), (tf.shape(decoder_month)[0], 1)
        )
        decode_idx = tf.cast(
            tf.one_hot(decode_idx, depth=24), tf.float32
        )  # batch * 12 * 12
        ...
        encoder_inputs = tf.concat(
            [
                ...
                t300_mean_lag2,
                encoder_month_embed,
                encoder_quarter_embed,
            ],
            axis=-1,
        )  # batch * 12 * 2
        decoder_inputs = tf.concat(
            [decoder_month_embed, decoder_quarter_embed, decode_idx], axis=-1
        )
...
def build_model():
    ...
    outputs = build_tfts_model(
        use_model="seq2seq", custom_model_params=model_params, dynamic_decoding=True
    )(enso_inputs)
...
def run_train():
    ...
    train_dataloader, valid_dataloader = build_data(
        base_dir="../tcdata", batch_size=fit_params["batch_size"]
    )  # prepare the data
    model = build_model()  # prepare the model
    trainer = Trainer(
        model=model, loss=custome_rmse_fn, optimizer=tf.keras.optimizers.Adam()
    )  # load it to trainer
